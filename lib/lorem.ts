...
export function lorem(words: number) {
  const start = Math.random() * LoremIpsum.length | 0 / 2
  return LoremIpsum.slice(start, start + words).join(' ')
}

export function loremRandomWord() {
  return LoremIpsum[Math.random() * LoremIpsum.length | 0]
    .replace(/\W/g, '')
}
